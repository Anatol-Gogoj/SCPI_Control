...
- **BK 4055B arbitrary waveforms: full-field `WVDT` header + TrueArb, short buffer.** The 4055B is an OEM SDG2000X; uploads follow the Siglent 16-bit-arb app note form with **every header field always present**: `C<n>:WVDT WVNM,<name>,FREQ,<f>,TYPE,8,AMPL,<a>,OFST,<o>,PHASE,<p>,WAVEDATA,<int16 LE>` — sample bytes follow `WAVEDATA,` directly (not an IEEE block), no `LENGTH`, no trailing terminator. **A minimal header (`WVNM,<name>,WAVEDATA,`) hard-wedges the box's SCPI parser over USBTMC** (bench-verified 2026-06-28 and 2026-07-02): writes are accepted into the void, every read times out, and only a front-panel power cycle recovers it — USBTMC `INITIATE_CLEAR`, `*RST`, USB reset, and even a VBUS port power-cycle all fail because the instrument is self-powered. (The minimal form does work over LAN, per the tinylabs SDG2000X library — the wedge is USB-specific.) Play short custom arbs via **TrueArb** (`C<n>:SRATE MODE,TARB,VALUE,<Sa/s>`), where output frequency = sample_rate ÷ num_points. `BK4055B.upload_arb` defaults to a **1024-point (~2 KB) buffer**; the old 32 KB DDS upload is what wedged the pyvisa-py USBTMC endpoint (issue #20). Self-test: `test_arb_scope.py` (sig gen → scope), `test_arb_upload_only.py` (wedge isolation), and headless `test_arb_transport.py`.
